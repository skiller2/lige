...
personalRouter.get(`${base}/datos-bot/nro-recibo/:personalId`, [authMiddleware.verifyToken, authMiddleware.hasAuthResp(true), authMiddleware.hasGroup([`Liquidaciones Consultas`, `Liquidaciones`])], (req, res, next) => {
  personalController.getUltimoNroReciboByPersonalId(req, res, next);
});

personalRouter.get(`${base}/datos-bot/:personalId`, [authMiddleware.verifyToken], (req, res, next) => {
