...
documentoRouter.get("/cols-download", [authMiddleware.verifyToken, authMiddleware.hasGroup(['gSistemas'])], (req, res) => {
  documentoController.getGridDownloadCols(req, res);
});

documentoRouter.get("/cols-no-download", [authMiddleware.verifyToken, authMiddleware.hasGroup(['gSistemas'])], (req, res) => {
  documentoController.getGridNoDownloadCols(req, res);
});

documentoRouter.get("/tipos/options", [authMiddleware.verifyToken, authMiddleware.hasGroup(['gSistemas'])], (req, res, next) => {
  documentoController.getTipos(req, res, next);
});

documentoRouter.post('/list', [authMiddleware.verifyToken, authMiddleware.hasGroup(['Liquidaciones', 'gSistemas'])], (req, res, next) => {
  documentoController.getdocgenralList(req, res, next)
})

documentoRouter.post('/list-download', [authMiddleware.verifyToken, authMiddleware.hasGroup(['Liquidaciones', 'gSistemas'])], (req, res, next) => {
  documentoController.getPersonalDescarga(req, res, next)
})

documentoRouter.post('/list-no-download', [authMiddleware.verifyToken, authMiddleware.hasGroup(['Liquidaciones', 'gSistemas'])], (req, res, next) => {
