...
valorHoraRouter.post("/changecellvalorHora", [authMiddleware.verifyToken, authMiddleware.hasGroup(['mValorHora'])], (req, res, next) => {
    valorHoraController.changecellvalorHora(req, res, next);
});

valorHoraRouter.post("/delete", [authMiddleware.verifyToken, authMiddleware.hasGroup(['mValorHora'])], (req, res, next) => {
    valorHoraController.deleteValorHora(req, res, next);
});

valorHoraRouter.post("/aumentar", [authMiddleware.verifyToken, authMiddleware.hasGroup(['mValorHora'])], (req, res, next) => {
