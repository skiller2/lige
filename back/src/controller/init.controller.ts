...
export class InitController extends BaseController {
  ...
  async getCustodiasPendientes(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      result.forEach(rec => {
        const PersonalId = (rec.ResponsableId) ? rec.ResponsableId : 0
  ...
  async getCustodiasLiquidadas(req: Request, res: Response, next: NextFunction) {
    const queryRunner = await getConnection(res.locals.userName)
    try {
      ...
      const custodiasLiquidadas: { x: string; y: number }[] = []
      let total = 0
      ...
      for (let i = 12; i >= 0; i--) {
        ...
        custodiasLiquidadas.push({ x: `${periodo.getMonth() + 1}/${periodo.getFullYear()}`, y: custodiasPeriodo.length })
        total += custodiasPeriodo.length
      }

      this.jsonRes({ custodiasLiquidadas, custodiasLiquidadasTotal: total }, res);
    } catch (error) {
      return next(error);
    } finally {
      await queryRunner.release()
...
GROUP BY suc.SucursalId, suc.SucursalDescripcion
  ...
  async getPersonasActivasSinHorasRegistradas(req: Request, res: Response, next: NextFunction) {
    const anio = Number(req.params.anio)
    const mes = Number(req.params.mes)
    const queryRunner = await getConnection(res.locals.userName)
    ...
    try {
      const rec = await queryRunner.query(`
        WITH HorasPorPersona AS (
          SELECT
            persona.PersonalId AS PersonalId,
            SUM(
              (
                ISNULL(CAST(LEFT(objd.ObjetivoAsistenciaAnoMesPersonalDias1Gral, 2) AS INT) * 60 + CAST(RIGHT(TRIM(objd.ObjetivoAsistenciaAnoMesPersonalDias1Gral), 2) AS INT), 0) +
                ISNULL(CAST(LEFT(objd.ObjetivoAsistenciaAnoMesPersonalDias2Gral, 2) AS INT) * 60 + CAST(RIGHT(TRIM(objd.ObjetivoAsistenciaAnoMesPersonalDias2Gral), 2) AS INT), 0) +
          ...
          FROM ObjetivoAsistenciaAnoMesPersonalDias objd
          JOIN ObjetivoAsistenciaAnoMes objm
            ON objm.ObjetivoAsistenciaAnoMesId = objd.ObjetivoAsistenciaAnoMesId
            AND objm.ObjetivoAsistenciaAnoId = objd.ObjetivoAsistenciaAnoId
            AND objm.ObjetivoId = objd.ObjetivoId
          JOIN ObjetivoAsistenciaAno obja
            ON obja.ObjetivoAsistenciaAnoId = objm.ObjetivoAsistenciaAnoId
            AND obja.ObjetivoId = objm.ObjetivoId
          JOIN Objetivo obj ON obj.ObjetivoId = obja.ObjetivoId
          JOIN Personal persona ON persona.PersonalId = objd.ObjetivoAsistenciaMesPersonalId
          JOIN PersonalCUITCUIL cuit
            ON cuit.PersonalId = persona.PersonalId
            AND cuit.PersonalCUITCUILId = persona.PersonalCUITCUILUltNro
          JOIN CategoriaPersonal cat
            ON cat.CategoriaPersonalId = objd.ObjetivoAsistenciaCategoriaPersonalId
            AND cat.TipoAsociadoId = objd.ObjetivoAsistenciaTipoAsociadoId
          LEFT JOIN ObjetivoHabilitacion objhab ON objhab.ObjetivoHabilitacionObjetivoId = obj.ObjetivoId
          WHERE obja.ObjetivoAsistenciaAnoAno = @0
            AND objm.ObjetivoAsistenciaAnoMesMes = @1
          GROUP BY persona.PersonalId
        ),
        PersonasActivas AS (
          SELECT DISTINCT per.PersonalId
          FROM Personal per
          JOIN PersonalSituacionRevista sitrev
            ON sitrev.PersonalId = per.PersonalId
            AND sitrev.PersonalSituacionRevistaDesde <= EOMONTH(DATEFROMPARTS(@0, @1, 1))
  ...
  async getPersonasActivasSinHabilitaciones(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      WITH PersonalSinHabilitacion AS (
        ...
        LEFT JOIN PersonalHabilitacion b ON b.PersonalId=per.PersonalId  and b.PersonalHabilitacionLugarHabilitacionId=vishab.LugarHabilitacionId and ((b.PersonalHabilitacionDesde <= @0 AND ISNULL(b.PersonalHabilitacionHasta, '9999-12-31') >= @0) or b.PersonalHabilitacionHasta is null or ISNULL(b.PersonalHabilitacionDesde, '9999-12-31')>=@0) 
        and ISNULL(b.PersonalHabilitacionClase, '') != 'C'

                LEFT JOIN PersonalHabilitacionNecesaria c ON c.PersonalId = per.PersonalId and c.PersonalHabilitacionNecesariaLugarHabilitacionId=vishab.LugarHabilitacionId
                LEFT JOIN LugarHabilitacion d ON d.LugarHabilitacionId = vishab.LugarHabilitacionId

                LEFT JOIN GestionHabilitacion e ON e.GestionHabilitacionCodigo = b.GestionHabilitacionCodigoUlt AND e.PersonalId = vishab.PersonalId AND e.PersonalHabilitacionLugarHabilitacionId = vishab.LugarHabilitacionId AND 
e.PersonalHabilitacionId = b.PersonalHabilitacionId
        ...
        LEFT JOIN GestionHabilitacionEstado est ON est.GestionHabilitacionEstadoCodigo = e.GestionHabilitacionEstadoCodigo
                LEFT JOIN (
                        SELECT
                                ...
                                CASE
                                        WHEN b2.PersonalHabilitacionHasta>=@0 THEN DATEDIFF(DAY, @0, b2.PersonalHabilitacionHasta)
                    --WHEN b2.PersonalHabilitacionHasta IS NULL and b2.PersonalHabilitacionDesde is null THEN NULL                                        --WHEN b2.PersonalHabilitacionHasta IS NULL AND b2.PersonalHabilitacionDesde IS NOT NULL THEN NULL
                                        ELSE 0
                                END AS DiasFaltantesVencimiento
                        FROM PersonalHabilitacion b2
                ) dias ON dias.PersonalId = vishab.PersonalId AND dias.PersonalHabilitacionId = b.PersonalHabilitacionId AND dias.PersonalHabilitacionLugarHabilitacionId = vishab.LugarHabilitacionId
        ...
        and ( (sitrev.PersonalSituacionRevistaSituacionId IN (2,10,12))  AND  ( IIF(b.PersonalId IS NULL, 0, dias.DiasFaltantesVencimiento) IN ('0'))  AND  (iif(e.GestionHabilitacionCodigo is null, 'PEN', e.GestionHabilitacionEstadoCodigo) LIKE '%PEN%') )
