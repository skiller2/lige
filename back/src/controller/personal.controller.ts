...
const columns: any[] = [
  ...
  },
  {
    ...
    type: "string",
    fieldName: "act.TipoPersonalActaCodigo",
...
export class PersonalController extends BaseController {
  ...
  async search(req: any, res: Response, next: NextFunction) {
    ...
    const params: any[] = [];
    let query: string = `SELECT per.PersonalId, cuit.PersonalCUITCUILCUIT, CONCAT(TRIM(per.PersonalApellido) , ', ', TRIM(per.PersonalNombre), ' CUIT:' , cuit.PersonalCUITCUILCUIT) fullName, ISNULL(sucper.PersonalSucursalPrincipalSucursalId,1) SucursalId
...
UNION ALL
        ...
        CASE
          WHEN pere.PersonalEstudioCursoId is null 
            THEN CONCAT(TRIM(TipoEstudioDescripcion) , ' - ', TRIM(pere.PersonalEstudioTitulo))
          WHEN pere.PersonalEstudioCursoId is not null 
            THEN CONCAT(TRIM(TipoEstudioDescripcion),' - ', ch.CursoHabilitacionDescripcion)
          ELSE pere.PersonalEstudioTitulo
        END as Descripcion,
        CONCAT('api/file-upload/downloadFile/', doc.DocumentoImagenEstudioId, '/DocumentoImagenEstudio/0') AS url,               
				CASE 
...
UNION ALL
  ...
  async getCategoriasByTipoAsociado(req: any, res: Response, next: NextFunction) {
    ...
    try {
      const options = await queryRunner.query(`
        ...
        WHERE catper.TipoAsociadoId IN (@0) AND isnull(catper.CategoriaPersonalInactivo, 0) = 0
        `, [tipoAsociado]
      )
  ...
  async searchTipoAsociadoCategoria(req: any, res: Response, next: NextFunction) {
    ...
    let query: string = `SELECT cat.TipoAsociadoId, cat.CategoriaPersonalId,
        ...
        FROM CategoriaPersonal cat 
        JOIN TipoAsociado tip ON tip.TipoAsociadoId = cat.TipoAsociadoId
        WHERE (CategoriaPersonalInactivo=0 or CategoriaPersonalInactivo IS NULL) and `;
    switch (fieldName) {
      case "Label":
        const valueArray: Array<string> = value.split(/[\s,.]+/);
        valueArray.forEach((element, index) => {
  ...
  async valsExeciones(queryRunner: any, form: any) {
    ...
    const PersonalExencionHasta: Date = form.PersonalExencionHasta ? new Date(form.PersonalExencionHasta) : null
    // let campos_vacios: any[] = [];
    let fieldErrors: any[] = [];
    ...
    if ((PersonalExencionId && !PersonalExencionDesde) || (!PersonalExencionDesde && PersonalExencionHasta)) {
  ...
  async addExenciones(req: any, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const ultPersonalExencion = await queryRunner.query(`
      ...
      await queryRunner.query(`
        ...
        `, [newPersonalExencionId, PersonalId, 1, PersonalExencionDesde, PersonalExencionHasta])
