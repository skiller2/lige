...
export class PersonalController extends BaseController {
  ...
  search(req: Request<{}, any, any, ParsedQs, Record<string, any>>, res: Response) {
    const connection = getConnection();
    const { fieldName, value } = req.body

    const valueArray: Array<string> = value.split(' ')

    let query: string = "SELECT persona.PersonalId, CONCAT(TRIM(persona.PersonalNombre) , ' ', TRIM(persona.PersonalApellido)) fullName FROM dbo.Personal persona WHERE"
