...
export class SegurosController extends BaseController {
  private async getPersonalBySitRev(queryRunner: any, anio: number, mes: number) {
    return await queryRunner.query(`	SELECT persr.PersonalId, persr.PersonalSituacionRevistaId, 
...
`, [ClientId, anio, mes])
  ...
  updateSeguros(anio: number, mes: number, options: any) {
    const queryRunner = dataSource.createQueryRunner();
    const personalCoto = this.getPersonalHorasByClientId(queryRunner, 1, anio, mes)
    const personalEdesur = this.getPersonalHorasByClientId(queryRunner, 798, anio, mes)
    const personalSitRev = this.getPersonalBySitRev(queryRunner, anio, mes)
