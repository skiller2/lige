...
recibosRouter.get("/download/:anio/:mes/:personalIdRel?", [authMiddleware.verifyToken, authMiddleware.hasAuthResp(true), authMiddleware.hasGroup(['Liquidaciones', 'Liquidaciones Consultas'])], (req, res, next) => {
...
recibosRouter.post('/downloadfull', [authMiddleware.verifyToken, authMiddleware.hasAuthObjetivo, authMiddleware.hasGroup(['Liquidaciones', 'Liquidaciones Consultas']), authMiddleware.filterSucursal], async (req, res, next) => {
