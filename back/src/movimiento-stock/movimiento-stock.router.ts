...
movimientoStockRouter.get("/tipos-destino",[authMiddleware.verifyToken, authMiddleware.hasGroup(['gSistemas'])], (req, res, next) => {
  movimientoStockController.getTiposDestino(req, res, next);
});

movimientoStockRouter.get("/tipos-origen",[authMiddleware.verifyToken, authMiddleware.hasGroup(['gSistemas'])], (req, res, next) => {
  movimientoStockController.getTiposOrigen(req, res, next);
});

movimientoStockRouter.get("/objetivo-info/:objetivoId/:anio/:mes", [authMiddleware.verifyToken, authMiddleware.hasGroup(['gSistemas'])], (req, res, next) => {
  movimientoStockController.getObjetivoInfo(req, res, next);
});

movimientoStockRouter.get("/proveedores", [authMiddleware.verifyToken, authMiddleware.hasGroup(['gSistemas'])], (req, res, next) => {
