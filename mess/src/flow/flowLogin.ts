import ChatBot from '@bot-whatsapp/bot'
...
const flowLogin = addKeyword(EVENTS.WELCOME)
    .addAction(async(ctx, {state, gotoFlow, flowDynamic}) => {
        ...
        if (res.length) {
            ...
            await flowDynamic(`Hola ${res[0].name.trim()}, bienvenido al área de consultas de Lince Seguridad`, {delay: delay})
...
const flowLoginOld = addKeyword(EVENTS.WELCOME)
    .addAction(async(ctx, {state, gotoFlow, flowDynamic}) => {
        const telefono = ctx.from
        console.log('state', state.getMyState());
