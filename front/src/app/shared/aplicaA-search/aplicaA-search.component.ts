...
@Component({
    selector: 'app-aplicaA-search',
    templateUrl: './aplicaA-search.component.html',
    styleUrls: ['./aplicaA-search.component.less'],
    imports: [
        ...SHARED_IMPORTS,
        CommonModule,
...
export class AplicaASearchComponent {
  ...
  onChange(item: any) {
    if(this.optionsArray.length > 0 ){
      const selectedItem = this.optionsArray.find(option => option.value === item);
      this.eto?.focus()  //Al hacer click en el componente hace foco nuevamente
      const selopt: any = this.optionsArray
      this.selectedId = item
      this.selectedItem = { id: item, fullName: selectedItem.label }
    }
  }
  ...
  ngOnInit() {
    ...
    this.searchService.getAplicaAOptions().subscribe((data: any[]) => {
