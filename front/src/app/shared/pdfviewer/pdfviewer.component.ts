...
export class PdfviewerComponent implements OnChanges {
  ...
  constructor(private downloadService: DownloadService, public sanitizer: DomSanitizer) {
    this.pdfDataUri = this.sanitizer.bypassSecurityTrustResourceUrl('');
  }
  ...
  async createPDF(bufferPDF: Uint8Array) {
    ...
    embededPages.forEach((embPage, index) => {
      ...
      currentPage.drawImage(embededImage, {
        x: 210,
        y: (index % 2 == 0 ? currentPage.getHeight() / 2 : 0) + 90,
