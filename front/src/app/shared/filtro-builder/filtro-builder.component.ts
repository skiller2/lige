...
export class FiltroBuilderComponent implements ControlValueAccessor {
  ...
  $optionsPersonalSexo = this.searchService.getPersonalSexoOptions();
  ...
  $optionsProveedores = this.searchService.getStockEfectoProveedores();
  ...
  selectedValueProveedor(val: any) {
    if (val) {
      this.selections.value = val.ProveedorId;
      this.valueExtended = { fullName: val.ProveedorRazonSocial };
    }
  }

  selectedValueRubro(val: any) {
    if (val) {
  ...
  async addFilter(index: string, condition: string, operator: string, value: any, label: string, closeable: boolean, inicial: boolean) {
    ...
    if (fieldObj.searchComponent == 'inputForEfectoSearch') {
      const efecto = await firstValueFrom(this.searchService.getEfectoFromName('StockReal', 'EfectoId', value, false, false, false))
