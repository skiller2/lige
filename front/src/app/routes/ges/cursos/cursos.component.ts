...
@Component({
  ...
  standalone: true,
  imports: [...SHARED_IMPORTS, CommonModule,TableCursoComponent]
})
export class CursosComponent {

  @ViewChild('cursosForm', { static: true }) cursosForm: NgForm = new NgForm([], []);
  

  
  PersonalId = signal<number>(0);
  PersonalCursoId = signal<number>(0);
  ...
  actualizarValorDrawer(event: any) {
    ...
    if (event.length > 0) {
      ...
      this.PersonalCursoId.set(event[0].PersonalCursoId);
    }
  }
