...
@Component({
  selector: 'app-table-instituciones',
  imports: [
    SHARED_IMPORTS,
    CommonModule,
    NzAffixModule,
  ...
  templateUrl: './table-instituciones.component.html',
  styleUrls: ['./table-instituciones.component.less'],
  standalone: true
})
export class TableInstitucionesComponent {
  ...
  columns$ = this.apiService.getCols('/api/instituciones/cols');
  RefreshInstituciones = input<boolean>(false);
  ...
  gridData$ = this.formChange$.pipe(
    debounceTime(250),
    switchMap(() => this.apiService.getListInstituciones({ options: this.listOptions }).pipe(
      map(data => {
