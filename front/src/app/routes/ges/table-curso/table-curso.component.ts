...
@Component({
  ...
  styleUrls: ['./table-curso.component.less'],
  standalone: true
})
export class TableCursoComponent {
  ...
  columns$ = this.apiService.getCols('/api/curso/cols');
  ...
  gridData$ = this.formChange$.pipe(
    debounceTime(250),
    switchMap(() => this.apiService.setListCursos({ options: this.listOptions }).pipe(
      map(data => {
