...
export class DepositosComponent {
  ...
  columns = toSignal(this.apiService.getCols('/api/depositos/cols'), { initialValue: [] as Column[] })
  ...
  gridData = resource({
    ...
    loader: async ({ params }) => {
      ...
      try {
        response = await firstValueFrom(this.apiService.getDepositos({options: params.options}));
  ...
  handleSelectedRowsChanged(e: any): void {
    if (e.detail.args.changedSelectedRows.length == 1) {
      ...
      const DepositoId = this.angularGrid.dataView.getItemByIdx(rowNum)?.id
      this.DepositoId.set(DepositoId)

    } else {
      this.DepositoId.set(0)
    }
  }
