...
@Component({
    selector: 'app-table-descuentos-stock',
    templateUrl: './table-descuentos-stock.component.html',
    styleUrls: ['./table-descuentos-stock.component.less'],
    encapsulation: ViewEncapsulation.None,
    providers: [AngularUtilService],
    imports: [ SHARED_IMPORTS, CommonModule, FiltroBuilderComponent ],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class TableDescuentosStockComponent {
    ...
    columns$ = this.apiService.getCols('/api/gestion-descuentos/cols/stock')

    gridData$ = this.listDescuento$.pipe(
        debounceTime(500),
        switchMap(() => {
            return this.apiService.getDescuentosStock(this.listOptions, this.anio(), this.mes())
                .pipe(map(data => { return data }))
        })
    )

    async ngOnInit() {
        this.gridOptions = this.apiService.getDefaultGridOptions('.gridDescStock', this.detailViewRowCount, this.excelExportService, this.angularUtilService, this, RowDetailViewComponent)
