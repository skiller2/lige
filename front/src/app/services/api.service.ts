...
export class ApiService {
  ...
  getDescuentoByPeriodo(year: number, month: number, personaIdRel: number): Observable<ResponseDescuentos> {
    ...
    const path = `/api/impuestos_afip/${year}/${month}/${personaIdRel}`;
  ...
  deleteCliente(ClienteId: number) {
    return this.http.delete<ResponseJSON<any>>(`/api/clientes/${ClienteId}`).pipe(
      tap((res: ResponseJSON<any>) => this.response(res)),
    )
  ...
  getListOrdenesVenta(options: any) {

    return this.http.post<ResponseJSON<any>>('/api/orden-venta/list-ordenes', { options }).pipe(
      map((res: { data: any; }) => res.data),
