...
export class ApiService {
  ...
  addAdelanto(adelanto: { PersonalId: string; monto: number; }) {
    if (!adelanto.PersonalId) { return throwError(() => new Error('Falta especificar la persona!')) }
    if (!adelanto.monto) { return throwError(() => new Error('Falta especificar el monto!'))}
    return this.http.post<ResponseJSON<any>>(`api/adelantos`, adelanto).pipe(
      map((res) => res.msg),
    )
  }
}
